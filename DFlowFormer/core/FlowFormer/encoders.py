...
class twins_svt_large(nn.Module):
    ...
    def forward(self, x, data=None, layer=2):
        ...
        for i, (embed, drop, blocks, pos_blk) in enumerate(
            ...
            # print('embed')
            # flops, params = profile(embed, inputs=(x,))
            ...
            # print('drop')
            # flops, params = profile(drop, inputs=(x,))
            # print('FLOPs = ' + str(flops / 1000 ** 3) + 'G')
            # print('Params = ' + str(params / 1000 ** 2) + 'M')
            # total = total + (flops / 1000 ** 3)
            # print('*' * 60)
            ...
            for j, blk in enumerate(blocks):
                # print('blk')
                # flops, params = profile(blk, inputs=(x, size,))
                # print('FLOPs = ' + str(flops / 1000 ** 3) + 'G')
                # print('Params = ' + str(params / 1000 ** 2) + 'M')
                # total = total + (flops / 1000 ** 3)
                # print('*' * 60)
                ...
                if j==0:
                    # print('pos_blk')
                    # flops, params = profile(pos_blk, inputs=(x, size,))
                    # print('FLOPs = ' + str(flops / 1000 ** 3) + 'G')
                    # print('Params = ' + str(params / 1000 ** 2) + 'M')
                    # total = total + (flops / 1000 ** 3)
                    # print('*' * 60)
                    x = pos_blk(x, size)
                    # print('pos_blk', pos_blk)
