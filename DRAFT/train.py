...
def fetch_optimizer(args, model):
    spatial_parameter_l = model.module.get_spatial_parameters()
    spatial_parameter_l_id = list(map(id, spatial_parameter_l))
    base_params = filter(lambda p: id(p) not in spatial_parameter_l_id, model.parameters())
    # params = [
    #     {"params": base_params, "lr": self.learing_rate},
    #     {"params": deformable_parameter_l, "lr": self.learing_rate * 0.1},
    # ]

    optimizer_spatial = optim.AdamW(spatial_parameter_l, lr=args.lr, weight_decay=args.wdecay, eps=args.epsilon)
    scheduler_spatial = optim.lr_scheduler.OneCycleLR(optimizer_spatial, args.lr, args.num_steps + 100,
                                              pct_start=0.05, cycle_momentum=False, anneal_strategy='linear')


    # optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.wdecay, eps=args.epsilon)
    optimizer = optim.AdamW(base_params, lr=args.lr, weight_decay=args.wdecay, eps=args.epsilon)
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, args.lr *0.4, args.num_steps+100,
        pct_start=0.05, cycle_momentum=False, anneal_strategy='linear')

    return optimizer, scheduler, optimizer_spatial, scheduler_spatial
...
class Logger:
    ...
    def write_sparsity_map(self, sparsity_map, image1, image2, flow, flow_predictions):
        ...
        for i in range(4):
            ...
            self.writer.add_image(str(i) + '/flow_predictions', flow_predictions_tmp, self.total_steps, dataformats='HWC')
...
def train(args):
    ...
    optimizer, scheduler, optimizer_spatial, scheduler_spatial = fetch_optimizer(args, model)
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            flow_predictions, sparsity, flow_predictions_wo_skip, inc_l = model(image1, image2, iters=args.iters, tau=tau, tgt_sparsity=tgt_sparsity, mhidden=mhidden)

            weight_sparsity = 5.
            # weight_sparsity = min((total_steps) / num_steps_weight, 1) * weight_sparsity
            loss, metrics = sequence_loss(flow_predictions, flow_predictions_wo_skip, inc_l, flow, valid, sparsity, tgt_sparsity_tmp, args.gamma, weight_sparsity, relu)
            scaler.scale(loss).backward()
            scaler.unscale_(optimizer)
            scaler.unscale_(optimizer_spatial)
